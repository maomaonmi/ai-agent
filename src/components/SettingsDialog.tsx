    ...
    <div role="dialog" aria-modal="true" aria-labelledby="settings-title" className="flex h-[min(820px,94vh)] w-full max-w-5xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto">
        ...
        {section === 'model' ? <div className="space-y-6 p-5 sm:p-7">
          ...
          {form.provider === 'minimax' && (
            <Field label="MiniMax 套餐 Key（tokenplan）" hint={form.has_minimax_plan_key ? '已保存套餐 Key（sk-cp-）；留空不覆盖。用于文本、搜索、PPT、图像和语音。' : '套餐 Key（sk-cp- 前缀，tokenplan）用于文本、搜索、PPT、图像和语音；视频 H3 使用上方普通 Key（sk-api-）。'}>
              <div className="relative">
                <Input type={showVideoKey ? 'text' : 'password'} value={form.minimax_plan_api_key || ''} placeholder={form.has_minimax_plan_key ? '••••••••（套餐 Key 已保存）' : '输入套餐 Key（sk-cp-）'} onChange={(v) => patch({minimax_plan_api_key:v})}/>
