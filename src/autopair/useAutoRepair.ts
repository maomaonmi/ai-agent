...
import { streamCode } from '../Code/streamCode'
import { injectErrorCatcher } from '../Code/injectErrorCatcher'
...
export function useAutoRepair() {
  ...
  const [history, setHistory] = useState<string[]>([]) // Day 51 新增：已应用的修改指令列表
  ...
  const generate = useCallback(
    (prompt: string) => {
      ...
      setRetryCount(0)
      setHistory([]) // 重新生成意味着开始新的一轮迭代，历史清空
  ...
  /**
   * Day 51 新增：迭代修改。
   * 不是"重新生成"，而是把当前代码 + 一句指令发给 /api/modify，
   * 模型只做增量修改，其余部分保持不变。
   */
  const modify = useCallback(
    (instruction: string) => {
      if (!instruction.trim() || !rawCodeRef.current) return
      setHistory((prev) => [...prev, instruction])
      setStatus({ state: 'modifying' })
      runStream('/api/modify', { code: rawCodeRef.current, instruction })
    },
    [runStream]
  )
  ...
  return { code: displayCode, status, retryCount, history, generate, modify }
}
