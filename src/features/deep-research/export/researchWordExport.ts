...
export async function createResearchWordDocument(title: string, report: string, sources: ResearchChunk[] = []): Promise<Blob> {
  const JSZip = (await import('jszip')).default;
  const zip = new JSZip();
  const references = sources.length ? `${paragraph('参考来源', 'Heading1')}${sources.map((source, index) => paragraph(`[${index + 1}] ${source.title || '未命名来源'} — ${source.url}`)).join('')}` : '';
  const body = `${paragraph(title || '深度调研报告', 'Title')}${paragraph(`生成日期：${new Intl.DateTimeFormat('zh-CN').format(new Date())}`)}${reportBody(report)}${references}`;
