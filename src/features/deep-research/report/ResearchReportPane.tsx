...
import { Check, Copy, Download, FileDown, Maximize2, Minimize2, Printer, X } from 'lucide-react';
...
export default function ResearchReportPane({ title, report, sources = [], loading = false, onClose }: ResearchReportPaneProps) {
  ...
  const downloadWord = async () => {
    try {
      const blob = await createResearchWordDocument(reportDocument.title, report, sources);
  ...
  return (
    <section ref={paneRef} aria-label="调研报告工作区" className="flex h-full min-h-0 flex-col bg-white text-slate-950 fullscreen:h-screen fullscreen:w-screen">
      <header className="relative z-20 flex h-16 shrink-0 items-center border-b border-slate-100 bg-white px-4 sm:px-5">
        <h2 className="min-w-0 max-w-[28%] truncate text-sm font-semibold">{title || '深度调研报告'}</h2>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-white">
        {!report.trim() ? <div role="status" aria-busy={loading} className="flex min-h-full items-center justify-center px-8 text-center"><div><div className="mx-auto h-10 w-10 rounded-full border border-slate-200 bg-slate-50 p-2">{loading ? <span className="block h-full w-full animate-spin rounded-full border-2 border-slate-200 border-t-blue-600"/> : <FileDown size={20} className="text-slate-400"/>}</div><h3 className="mt-4 text-sm font-medium text-slate-800">{loading ? '正在生成调研报告' : '报告将在调研完成后显示'}</h3><p className="mt-2 text-xs leading-5 text-slate-400">左侧对话和调研链路会持续保留。</p></div></div> : view === 'deep' ? <DeepResearchDocument document={reportDocument}/> : <RawResearchReport title={title} report={report}/>} 
      </div>
    </section>
  );
}
