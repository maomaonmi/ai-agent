...
import { BarChart3, BookOpenCheck, LoaderCircle, PieChart, TrendingUp, Zap } from 'lucide-react';
import MarkdownMessage from '../../../components/MarkdownMessage';
import type { ResearchFigure } from '../../../lib/api';
...
function ResearchFigureBlock({ figure, onLoadError, onRetry }: { figure: ResearchFigure; onLoadError?: (figureId: string) => void; onRetry?: (figureId: string) => void }) {
  const ready = figure.status === 'succeeded' && Boolean(figure.image_url);
  return <figure data-research-figure className="my-8 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-sm sm:max-w-[44rem]">
    {ready ? <img src={figure.image_url ?? ''} alt={figure.caption} loading="lazy" onError={() => onLoadError?.(figure.id)} className="block aspect-[4/3] w-full object-cover"/> : <div className="flex aspect-[4/3] flex-col items-center justify-center gap-3 bg-gradient-to-br from-slate-100 via-white to-blue-50 px-8 text-center text-sm text-slate-400">{figure.status === 'failed' ? <><span>{figure.error_message || '配图生成失败'}，正文仍可正常阅读</span>{onRetry && <button type="button" onClick={() => onRetry(figure.id)} className="rounded-md border border-blue-200 bg-white px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-50">重试配图</button>}</> : <><LoaderCircle size={24} className="animate-spin text-blue-500"/><span>正在生成研究配图…</span></>}</div>}
    <figcaption className="flex items-center justify-between gap-3 px-4 py-3 text-xs text-slate-500"><span>{figure.caption}</span><span className="shrink-0 text-[11px] text-slate-400">{figure.figure_type}</span></figcaption>
  </figure>;
}

function ReportBody({ markdown, charts, figures, onFigureLoadError, onFigureRetry }: { markdown: string; charts: ResearchDataChart[]; figures: ResearchFigure[]; onFigureLoadError?: (figureId: string) => void; onFigureRetry?: (figureId: string) => void }) {
  ...
  const figureMap = useMemo(() => figures.reduce<Record<number, ResearchFigure[]>>((map, figure, index) => { const sectionIndex = Math.min(sections.length - 1, Math.max(0, Math.round(((index + 1) * sections.length) / (figures.length + 1)) - 1)); (map[sectionIndex] ??= []).push(figure); return map; }, {}), [figures, sections.length]);
  return <div data-research-markdown className="research-markdown-document">{sections.map((section, index) => <div key={`${index}-${section.slice(0, 20)}`}><MarkdownMessage content={section} className="text-[15px] leading-8 text-slate-800 [&_h1]:mt-10 [&_h1]:text-2xl [&_h2]:mt-10 [&_h2]:border-b [&_h2]:border-slate-200 [&_h2]:pb-3 [&_h2]:text-xl [&_h3]:mt-8 [&_h3]:text-lg"/>{chartMap[index]?.map((chart) => <EvidenceChart key={`${chart.title}-${chart.labels.join('-')}`} chart={chart}/>)}{figureMap[index]?.map((figure) => <ResearchFigureBlock key={figure.id} figure={figure} onLoadError={onFigureLoadError} onRetry={onFigureRetry}/>)}</div>)}</div>;
}

export default function DeepResearchDocument({ document, figures = [], onFigureLoadError, onFigureRetry }: { document: ResearchReportDocument; figures?: ResearchFigure[]; onFigureLoadError?: (figureId: string) => void; onFigureRetry?: (figureId: string) => void }) {
  return <article data-research-document className="mx-auto max-w-[980px] px-6 pb-20 pt-12 sm:px-10 lg:px-12"><header className="mb-8 text-center"><div className="mb-5 flex items-center justify-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em]"><span className="rounded bg-slate-950 px-3 py-1.5 text-white">Deep Research</span><span className="rounded bg-blue-600 px-3 py-1.5 text-white">深度研报</span></div><h1 className="text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">{document.title}</h1><p className="mt-4 text-sm text-slate-500">生成日期：{new Intl.DateTimeFormat('zh-CN').format(new Date())}</p></header>
    ...
    <ReportBody markdown={document.markdown} charts={document.dataCharts} figures={figures} onFigureLoadError={onFigureLoadError} onFigureRetry={onFigureRetry}/>
