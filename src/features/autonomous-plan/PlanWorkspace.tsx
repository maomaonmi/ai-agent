...
import { Check, Clipboard, Download, Expand, FileText, ListTree, PanelRightClose, X } from 'lucide-react';
import { createPlanFigureJob, getPlanFigureJob, retryPlanFigure, type PlanFigure, type PlanProgressEvent, type PlanFigureJob } from '../../lib/api';
...
export default function PlanWorkspace({ progress, report, figures = [], loading, distributed, sidebarCollapsed, sessionId, onWidthChange, onFiguresChange }: PlanWorkspaceProps) {
  ...
  const reportDocument = useMemo(() => adaptPlanReport(report, figures, progress), [report, figures, progress]);
  useEffect(() => {
      ...
      try {
        ...
        if (usableJobId(existingJobId)) {
          try {
            ...
            job = await createPlanFigureJob({ session_id: sessionId, report_version: key, report, max_images: 10, policy: 'economy', context_mode: 'mixed' });
          }
        } else {
          job = await createPlanFigureJob({ session_id: sessionId, report_version: key, report, max_images: 10, policy: 'economy', context_mode: 'mixed' });
        }
        ...
        const update = (next: PlanFigureJob) => {
          const nextFigures = (next.figures || []).map((figure) => timedOutFigures.current.has(figure.id)
            ...
            : figure);
          if (nextFigures.length) onFiguresChangeRef.current?.(nextFigures);
  ...
  const handleRetryFigure = (figureId: string) => {
    ...
    const request = usableJobId(current.id)
      ...
      : createPlanFigureJob({ session_id: sessionId, report_version: `plan-retry-${report.length}`, report, max_images: 10, policy: 'economy', context_mode: 'mixed' });
