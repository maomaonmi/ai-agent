...
function Chart({ chart }: { chart: PlanReportChart }) {
  ...
  return <div data-plan-chart={chart.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-800">{chart.kind === 'line' ? <LineChart size={16} /> : <BarChart3 size={16} />}{chart.title}</div>
    <div className="flex h-36 items-end gap-2" role="img" aria-label={`${chart.title} ${chart.kind === 'line' ? '折线图' : '柱状图'}`}>
      {chart.values.map((value, index) => <div key={`${chart.labels[index]}-${index}`} className="group relative flex h-full flex-1 flex-col justify-end">
        ...
        <div tabIndex={0} className={`w-full rounded-t-md transition-all focus:outline-none focus:ring-2 focus:ring-blue-400 ${chart.kind === 'line' ? 'bg-gradient-to-t from-violet-500 to-fuchsia-400' : 'bg-gradient-to-t from-blue-600 to-cyan-400'}`} style={{ '--chart-h': `${Math.max(8, (value / max) * 110)}px`, height: `max(8px, ${(value / max) * 110}px)` } as React.CSSProperties} />
...
function FigureSlot({ figure, onRetry }: { figure: PlanFigure; onRetry?: (figureId: string) => void }) {
  ...
  return <figure data-plan-figure className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <figcaption className="flex items-center gap-2 px-4 py-3 text-xs text-slate-500"><ImageIcon size={14} />{figure.caption || '任务报告配图'}</figcaption>
  </figure>;
}
...
export default function PlanReportDocument({ document, onRetryFigure }: { document: PlanReportDocument; onRetryFigure?: (figureId: string) => void }) {
  return <article data-plan-report className="space-y-6 pb-8 text-slate-800">
    ...
    {document.sections.map((section, index) => <section key={`${section.heading}-${index}`} className="space-y-4"><h2 className="border-b border-slate-200 pb-2 text-xl font-semibold">{section.heading}</h2><MarkdownMessage className="text-[15px] leading-7" content={section.body} />{document.charts[index] && <Chart chart={document.charts[index]} />}{document.figures[index] && <FigureSlot figure={document.figures[index]} onRetry={onRetryFigure} />}</section>)}
    ...
    {document.tables.length > 0 && <section className="space-y-4"><h2 className="text-xl font-semibold">关键数据表</h2>{document.tables.map((table, index) => <div key={index} className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm"><table className="min-w-full text-left text-sm"><thead className="bg-slate-100"><tr>{table.headers.map((header) => <th key={header} className="px-4 py-3 font-semibold">{header}</th>)}</tr></thead><tbody>{table.rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-slate-100 hover:bg-slate-50">{row.map((cell, cellIndex) => <td key={cellIndex} className="px-4 py-3 align-top">{cell}</td>)}</tr>)}</tbody></table></div>)}</section>}
  </article>;
}
