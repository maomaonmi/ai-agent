...
test_cases = [
    {
        "query": "我手机掉水里了，修一下贵吗？",
        "expected_point": "必须提到进水维修200元起，且需拆机清理。"
    },
    {
        "query": "我是张大炮，我要修屏幕。帮我看看钱够吗？",
        "expected_point": "必须识别出张大炮余额100元，屏幕500元，结论是钱不够。"
    },
    {
        "query": "你们这儿能升级内存吗？要多久？",
        "expected_point": "必须提到内存升级300元，但由于资料未提时间，必须诚实回答时间未知。"
    }
]

def run_judge_node(state: GroundedState):
    print("[Node: Judge] 🏛️  正在进行最终评分...")
    ...
    评分标准（1-10分）：
    1. 事实性 (Factuality)：回答是否完全符合资料？多说或少说、说错均扣分。
    2. 引用 (Citations)：是否在每个事实后准确标注了 [n]？
    3. 诚实性 (Honesty)：资料没提的内容，AI 是否承认不知道？
    ...
    try:
        ...
        print(f"  📊 最终评分: {result_data['score']}/10 | 幻觉: {result_data['hallucination']}")
        print(f"  📋 理由: {result_data['reason']}")
    except Exception:
        print(f"  ⚠️ 评分解析失败: {res.content}")
...
def should_continue(state: GroundedState) -> Literal["analyst", "judge"]:
...
workflow.add_node("judge", run_judge_node)
