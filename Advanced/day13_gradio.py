...
# 审批后用的版本：不带中断，一次跑到底
app_no_interrupt = workflow.compile(checkpointer=memory_storage)
...
def chat_with_agent(user_message, chat_history):
    # 使用固定的 thread_id，这样 Gradio 每次调用都能接着上次的对话
    thread_id = "user_001"
    ...
    snapshot = app.get_state(config)
    if snapshot.next:
        # 处于中断状态：看用户是不是输入了审批通过
        if "审批通过" in user_message:
            # 审批通过后，用不带中断的 app 一次跑到底
            final_response = ""
            for event in app_no_interrupt.stream(None, config=config):
                for node_name, values in event.items():
            ...
            return final_response if final_response else "✅ 审批完成，任务已执行！"
        else:
            return "⏸️ 系统正在等待你的审批，请输入 审批通过 以继续..."
            
    # --- 正常流程：把用户消息发给 LangGraph ---
    final_response = ""
    for event in app.stream({"messages": [HumanMessage(content=user_message)]}, config=config):
        for node_name, values in event.items():
            if "messages" in values:
                last_msg = values["messages"][-1]
                if hasattr(last_msg, "content") and isinstance(last_msg.content, str):
                    content = last_msg.content
                    # 跳过路由标签（经理节点的分配结果），只保留人话
                    if content not in ["BALANCE_AGENT", "TECH_AGENT", "FINISH"]:
                        final_response = content

    # --- 流结束后，检查是否被中断（即将执行工具，需要审批） ---
    snapshot = app.get_state(config)
    if snapshot.next:
        final_response += "\n\n⏸️ **需要人工审批才能执行工具**"
        final_response += "\n请输入 ✅ 审批通过 以继续"

    return final_response if final_response else "AI 正在思考中..."

# Gradio 提供了一个专门的 ChatInterface 类，三行代码就能造个网页
demo = gr.ChatInterface(
    fn=chat_with_agent,
    title="👔 黑科技手机管家 Pro",
    description="支持多专家协作与人工审批的智能 Agent"
)
demo.launch()
