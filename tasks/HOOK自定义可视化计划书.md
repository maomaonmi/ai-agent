...
1. HOOK 的第一版是否只面向本地单用户，暂不做多用户权限和团队共享？本计划按“本地单用户”设计。
2. 第一版是否接受只支持内置/声明式 HOOK，不开放任意 Python/JavaScript？这是安全上的强建议。
3. 运行历史默认保留多久？若未指定，建议只保留摘要和最近 100 次 run。
4. 实时观察是否需要默认展开？建议默认折叠，用事件徽标提示，避免干扰主任务。
