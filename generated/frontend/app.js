...
const MOCK_PLAYLISTS = [
  { id: 1, title: "深夜电子", cover: "https://picsum.photos/seed/pl1/400/400", count: 32 },
  { id: 2, title: "清晨轻音乐", cover: "https://picsum.photos/seed/pl2/400/400", count: 28 },
  { id: 3, title: "运动节拍", cover: "https://picsum.photos/seed/pl3/400/400", count: 45 },
  { id: 4, title: "雨天氛围", cover: "https://picsum.photos/seed/pl4/400/400", count: 19 },
  { id: 5, title: "复古摇滚", cover: "https://picsum.photos/seed/pl5/400/400", count: 36 },
  { id: 6, title: "国风古韵", cover: "https://picsum.photos/seed/pl6/400/400", count: 24 }
];
...
function renderTracks() {
  elements.tracksGrid.innerHTML = state.tracks.map(track => `
    <div class="track-card ${state.currentTrack?.id === track.id ? 'active' : ''}" 
      ...
      <button class="like-btn ${state.favorites.has(track.id) ? 'liked' : ''}" data-id="${track.id}">${state.favorites.has(track.id) ? '♥' : '♡'}</button>
...
async function likeTrack(trackId) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/tracks/${trackId}/like`, { method: 'POST' });
    if (!response.ok) throw new Error('Network response was not ok');
    const data = await response.json();
    console.log('Like recorded:', data);
  } catch (error) {
    console.warn('Backend not available, like recorded locally:', error.message);
    const track = state.tracks.find(t => t.id === trackId);
    if (track) {
      track.likes = (track.likes || 0) + 1;
      console.log('Mock like recorded locally, total likes:', track.likes);
    }
  }
}
...
async function recordPlay(trackId) {
  try {
    ...
    console.warn('Backend not available, play recorded locally:', error.message);
    const track = state.tracks.find(t => t.id === trackId);
    if (track) {
...
function initNav() {
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      const tab = link.textContent.trim();
      const tabMap = { '发现': 'discover', '歌单': 'playlist', '收藏': 'favorites' };
      const targetTab = tabMap[tab];
      if (!targetTab || targetTab === state.currentTab) return;
      state.currentTab = targetTab;
      navLinks.forEach(l => l.classList.remove('active'));
      link.classList.add('active');
      document.getElementById('discoverSection').style.display = targetTab === 'discover' ? '' : 'none';
      document.getElementById('playlistSection').style.display = targetTab === 'playlist' ? '' : 'none';
      document.getElementById('favoritesSection').style.display = targetTab === 'favorites' ? '' : 'none';
...
function renderPlaylists() {
  ...
  grid.innerHTML = MOCK_PLAYLISTS.map(pl => `
    <div class="playlist-card" data-id="${pl.id}">
      <img src="${pl.cover}" alt="${pl.title}" class="playlist-card-cover">
      <div class="playlist-card-info">
        <div class="playlist-card-title">${pl.title}</div>
        <div class="playlist-card-count">${pl.count} 首歌曲</div>
      </div>
    </div>
  `).join('');
  grid.querySelectorAll('.playlist-card').forEach(card => {
    card.addEventListener('click', () => {
      if (state.tracks.length > 0) {
        selectTrack(state.tracks[Math.floor(Math.random() * state.tracks.length)]);
      }
    });
  });
}
...
function renderFavorites() {
  ...
  grid.innerHTML = favTracks.map(track => `
    <div class="track-card ${state.currentTrack?.id === track.id ? 'active' : ''}" data-id="${track.id}">
      <img src="${track.cover}" alt="${track.title}" class="track-card-cover">
      <div class="track-card-info">
        <div class="track-card-title">${track.title}</div>
        <div class="track-card-artist">${track.artist}</div>
      ...
      <div class="track-card-duration">${formatTime(track.duration)}</div>
      <button class="like-btn liked" data-id="${track.id}">♥</button>
      <div class="track-card-play">
        <svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
      </div>
    </div>
