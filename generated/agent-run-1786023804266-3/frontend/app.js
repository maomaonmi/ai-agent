...
document.addEventListener('keydown', (e) => {
  // 子类别点击事件绑定
  document.querySelectorAll('.cz-sub').forEach(sub => {
    sub.addEventListener('click', function(e) {
      e.stopPropagation();
      const subName = this.textContent.trim();
      const parentCard = this.closest('.category-zone-card');
      const catId = parentCard.dataset.cat;
